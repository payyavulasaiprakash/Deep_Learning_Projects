...
def draw_axis(img, yaw, pitch, roll, tdx=None, tdy=None, size = 50):
    ...
    cv2.putText(img, "pitch: " + str(np.round(pitch,2)), (300, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    cv2.putText(img, "yaw: " + str(np.round(yaw,2)), (300, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    cv2.putText(img, "roll: " + str(np.round(roll,2)), (300, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
...
def draw_results_ssd(detected,input_img,faces,ad,img_size,img_w,img_h,model,time_detection,time_network,time_plot,img_idx):
    ...
    if detected.shape[2]>0:
        for i in range(0, detected.shape[2]):
            ...
            if confidence > 0.5:
                ...
                # cv2.imwrite('img/'+str(img_idx)+f'_{str(i)}_fd_with_extra_ssd.png', input_img[yw1:yw2 + 1, xw1:xw2 + 1, :])
                faces[i,:,:,:] = cv2.resize(input_img[yw1:yw2 + 1, xw1:xw2 + 1, :], (img_size, img_size))
                # cv2.imwrite('img/'+str(img_idx)+f'_{str(i)}_resized_ssd.png', faces[i,:,:,:])
...
def main():
    ...
    while True:
        ...
        if img_idx==1 or img_idx%skip_frame == 0:
            ...
            input_img = draw_results_ssd(detected,input_img,faces,ad,img_size,img_w,img_h,model,time_detection,time_network,time_plot,img_idx)
            # cv2.imwrite('img/'+str(img_idx)+'_ssd.png',input_img)
