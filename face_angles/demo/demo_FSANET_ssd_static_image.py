...
def draw_axis(image_name,img, yaw, pitch, roll, tdx=None, tdy=None, size = 50):
    print(image_name,yaw,roll,pitch)

    cv2.putText(img, "pitch: " + str(np.round(pitch,2)), (0, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 1)
    cv2.putText(img, "yaw: " + str(np.round(yaw,2)), (0, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 1)
    cv2.putText(img, "roll: " + str(np.round(roll,2)), (0, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 1)
...
def draw_results_ssd(detected,input_img,faces,ad,img_size,img_w,img_h,model,time_detection,time_network,time_plot,image_name,des_folder_path_2):
    ...
    if detected.shape[2]>0:
        for i in range(0, detected.shape[2]):
            ...
            if confidence > 0.5 and i==0:
                ...
                # print('ad,xw1,yw1,xw2,yw2 ',ad,xw1,yw1,xw2,yw2)
                # cv2.imshow('check_image',cv2.resize(input_img[yw1:yw2 + 1, xw1:xw2 + 1, :], (img_size, img_size)))
                faces[i,:,:,:] = cv2.resize(input_img[yw1:yw2 + 1, xw1:xw2 + 1, :], (img_size, img_size))
                cv2.imwrite(os.path.join(des_folder_path_2,image_name+f"_{i}detection.jpg"),faces[i,:,:,:])
                ...
                face = face.squeeze()
                img = draw_axis(image_name,input_img[yw1:yw2 + 1, xw1:xw2 + 1, :], p_result[0][0], p_result[0][1], p_result[0][2])
...
def main():
    ...
    main_folder_path = '' #folder path
    des_folder_path = main_folder_path + '_ssd2' #output images(on which yaw pitch roll will be written) will be saved in this folder
    des_folder_path_2 = des_folder_path + '_model_pass'   #images that go to the model(FSA net) will be saved in this folder
    main_folder_images_path = glob.glob(main_folder_path+'/*') #check it according to test the data
    try:
        os.makedirs(des_folder_path)
        os.makedirs(des_folder_path_2)
    except OSError:
        pass
    ...
    for img_path in main_folder_images_path:
        try:
            ...
            if img_idx==1 or img_idx%skip_frame == 0:
                ...
                # print('detected',detected.shape,np.shape(input_img))
                if detected_pre.shape[2] > 0 and detected.shape[2] == 0:
                    detected = detected_pre
                    detection_flag = "not_detected"

                faces = np.empty((detected.shape[2], img_size, img_size, 3))

                input_img = draw_results_ssd(detected,input_img,faces,ad,img_size,img_w,img_h,model,time_detection,time_network,time_plot,image_name,des_folder_path_2)
                cv2.imwrite(des_folder_path+'/'+str(image_name)+f'_{detection_flag}_ssd.png',input_img)
